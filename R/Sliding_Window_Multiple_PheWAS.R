		...
		for (num in 1:length(obj_nullmodel_list))
		{
			...
			for(k in 1:sliding_window_num)
			{
				...
				if(n_pheno == 1)
				{
					if(!use_SPA)
					{
						try(pvalues <- STAAR(G,obj_nullmodel_list[[num]],phred_sub,rare_maf_cutoff=rare_maf_cutoff,rv_num_cutoff=rv_num_cutoff,rv_num_cutoff_max=rv_num_cutoff_max),silent=silent)
					}else
					{
						try(pvalues <- STAAR_Binary_SPA(G,obj_nullmodel_list[[num]],phred_sub,rare_maf_cutoff=rare_maf_cutoff,rv_num_cutoff=rv_num_cutoff,rv_num_cutoff_max=rv_num_cutoff_max,SPA_p_filter=SPA_p_filter,p_filter_cutoff=p_filter_cutoff),silent=silent)
					}
				}else
				{
					try(pvalues <- MultiSTAAR(G,obj_nullmodel_list[[num]],phred_sub,rare_maf_cutoff=rare_maf_cutoff,rv_num_cutoff=rv_num_cutoff,rv_num_cutoff_max=rv_num_cutoff_max),silent=silent)
				}
