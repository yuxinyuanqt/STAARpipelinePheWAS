#' Genetic region PheWAS analysis of sliding windows using STAAR procedure
#'
...
#' @return A list of data frames containing the STAAR p-values (including STAAR-O or STAAR-B in imbalanced case-control setting) corresponding to each sliding window in the given genetic region for each phenotype.
...
Sliding_Window_PheWAS <- function(chr,start_loc,end_loc,sliding_window_length=2000,type=c("single","multiple"),
                                  genofile,obj_nullmodel_list,rare_maf_cutoff=0.01,rv_num_cutoff=2,
	...
	if(type=="single")
	{
		results_list <- Sliding_Window_Single_PheWAS(chr=chr,start_loc=start_loc,end_loc=end_loc,
		                                             genofile=genofile,obj_nullmodel_list=obj_nullmodel_list,
	...
	if(type=="multiple")
	{
		results_list <- Sliding_Window_Multiple_PheWAS(chr=chr,start_loc=start_loc,end_loc=end_loc,
